    ...
    public class Hashing
    {
        ...
        public static byte[] GenerateHashCode(List<byte[]> blocks)
        {
            ...
            for(int i = 0; i < blocks.Count - 1; i++)
                md5.TransformBlock(blocks[i], 0, blocks[i].Length, null, 0);
        ...
        {
            var hashFunction = SHA256.Create("System.Security.Cryptography.SHA256CryptoServiceProvider");

            for (int i = 0; i < blocks.Length - 1; i++)
                hashFunction.TransformBlock(blocks[i], 0, blocks[i].Length, null, 0);

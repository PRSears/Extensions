...
public class CsvSerializer<T>
{
        ...
        return properties.Where
                          (
                              p => p.GetCustomAttributes(false)
                                    .Count(a => a is System.Xml.Serialization.XmlIgnoreAttribute) <
                                   1
                          )
                         .ToArray();
    }
    ...
    {
        return GetPublicProperties().Select(p => p.Name).ToArray();
    }
    ...
    public void Serialize(FileStream stream, T o)
    {
        ...
        if (stream.Length == 0)
        {
            ...
            writer.WriteLine(string.Join(",", PublicPropertyNames));
        }
        else
        {
        ...
        {
            object? propertyValue = PublicProperties[i].GetValue(o);
    ...
    public T[] Deserialize(FileStream stream)
    {
        ...
        List<T> data = new();
        while (reader.Peek() > -1) data.Add(Deserialize(reader.ReadLine()));
    ...
    public IEnumerable<T> Deserialize(FileStream stream, Func<string, bool> predicate)
    {
        ...
        return File.ReadLines(tmp).Where(predicate).Select(s => Deserialize(s));
    }
    ...
    protected T Deserialize(string[] item)
    {
        ...
        if (item.Length != head.Length)
            Debug.WriteMessage
            (
                $"[{GetType().Name}] Number of properties in header "             +
